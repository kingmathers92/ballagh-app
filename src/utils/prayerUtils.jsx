...
export const scheduleReminders = (
  ...
  Object.keys(notifiedToday).forEach((key) => {
    if (!key.startsWith(todayKey)) delete notifiedToday[key];
  });

  const reminderablePrayers = [
    { name: "fajr", time: rawTimes.fajr },
    { name: "dhuhr", time: rawTimes.dhuhr },
    { name: "asr", time: rawTimes.asr },
    { name: "maghrib", time: rawTimes.maghrib },
  ...
  reminderablePrayers.forEach(({ name, time }) => {
    if (!prayerReminders[name]) return;

    const key = `${todayKey}-${name}`;
    if (notifiedToday[key]) return;

    const msSincePrayer = now.getTime() - time.getTime();
    if (msSincePrayer >= 0 && msSincePrayer <= 90 * 1000) {
      notifiedToday[key] = true;

      const prayerLabel = translations[language]?.prayers?.[name] || name;
      ...
      if (
        ...
        try {
          new Notification(`${prayerLabel} \u2014 time to pray`, {
            body: timeLabel,
            icon: "/pwa-192x192.png",
            tag: key,
          });
        } catch (err) {
          console.error("Error showing prayer notification:", err);
        }
      }
